...
export const TabsList = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "inline-flex h-10 items-center justify-center rounded-[var(--radius-md)] bg-[var(--color-neutral-100)] p-1 text-[var(--color-neutral-500)]",
        className
      )}
      {...props}
    />
  ...
  ({ className, value, ...props }, ref) => {
    ...
    return (
      <button
        ...
        className={cn(
          "inline-flex items-center justify-center whitespace-nowrap rounded-sm px-3 py-1.5 text-[var(--text-body-sm-size)] font-medium ring-offset-[var(--background)] transition-all duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary-500)] focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50",
          isActive
            ? "bg-[var(--background)] text-[var(--foreground)] shadow-sm"
            : "hover:bg-[var(--color-neutral-200)] hover:text-[var(--color-neutral-800)]",
          className
        )}
        {...props}
      />
